...
export default function CreateProductPage() {
    ...
    return (
        <div>
            <h3 className="text-2xl font-bold text-gray-800 mb-6">Create Product</h3>
            ...
            <form onSubmit={handleProductSubmit} className="flex flex-col gap-5">
                <div>
                    ...
                    <div className="border border-dashed border-gray-300 rounded-lg p-4 bg-gray-50 text-center hover:bg-gray-100 transition cursor-pointer">
                        <input 
                            type="file" 
                            name="cover_image" 
                            accept="image/*" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Product First Name</label>
                        <input 
                            type="text" 
                            name="firstName" 
                            required 
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Product Last Name</label>
                        <input 
                            type="text" 
                            name="lastName" 
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-gray-700" 
                ...
                <div className="border-t pt-5">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="text-lg font-semibold text-gray-800">Product Weights</h4>
                        <button 
                            type="button" 
                            onClick={addWeight}
                    ...
                    {weights.map((weight, index) => (
                        <div key={index} className="bg-gray-50 p-4 rounded-lg mb-4 border">
                            <div className="flex items-center justify-between mb-3">
                                <span className="text-sm font-medium text-gray-600">Weight {index + 1}</span>
                                {weights.length > 1 && (
                                    <button 
                                        type="button" 
                            ...
                            <div>
                                <label className="block text-xs text-gray-500 mb-1">Images for {weight.weight || 'this weight'}</label>
                                <div className="border border-dashed border-gray-300 rounded-lg p-3 bg-white text-center">
                                    <input 
                                        type="file" 
                                        multiple 
                                        accept="image/*"
                                        onChange={(e) => handleWeightImageChange(index, e)}
                                        className="w-full text-sm text-gray-500 file:mr-2 file:py-1 file:px-2 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100" 
                                    />
                                </div>
                ...
                <button type="submit" disabled={loading} className="mt-4 flex items-center justify-center gap-2 w-full bg-emerald-600 text-white font-medium py-3 rounded-lg shadow hover:bg-emerald-700 disabled:opacity-70 transition">
                    <FiUploadCloud className="w-5 h-5" /> {loading ? "Uploading..." : "Upload Product"}
                </button>
            </form>
        </div>
    );
